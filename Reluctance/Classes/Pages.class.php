...
//		self::$limit 		= ' where id > ' . ceil(self::$offset/2) . ' limit ' . self::$offset . ',' . self::$num ;
	...
	{
		return 	self::$page <= 1     ?     '<span class="disabled">首页</span><span class="disabled">上一页</span>'     :     '<a href="'.self::$url.'/page/1">首页</a>' . '<a href="'.self::$url.'/page/'.self::$prevNum.'">上一页</a>';

	}

	STATIC protected function getNextPage()
	{
		return	self::$page < self::$pages    ?     '<a href="'.self::$url.'/page/'.self::$nextNum.'">下一页</a>' . '<a href="'.self::$url.'/page/'.self::$pages.'">尾页</a>'      :      '<span class="disabled">下一页</span><span class="disabled">尾页</span>';
	}
	...
	STATIC protected function getNumStart($NumPageNum)
	{
		...
		for($i=$m;$i<=$n; $i++){
			if($i ==0) continue;
			$page .= self::$page == $i ?  '<span class="current">'.$i.'</span>' : '<a href="'.self::$url.'/page/'.$i.'">'.$i.'</a>';
		}
		return $page;
	}
	...
	{
		$page = "转到<input type='text' size='3' title='请输入要跳转到的页数并回车' onkeyup=\"this.value=this.value.replace(/\D/g,'')\" onafterpaste=\"this.value=this.value.replace(/\D/g,'')
\" onkeydown=\"javascript:if(event.charCode==13||event.keyCode==13){if(!isNaN(this.value)){document.location.href='".self::$url."/page/'+this.value+'';}return false;}\"/>页";
		return $page;
	}

	STATIC protected function getSelectStart( $selectNum )
	{
		$page 	= "跳至<select name='topage' size='1' onchange='window.location=\"".self::$url."/page/\"+this.value'>\n";
